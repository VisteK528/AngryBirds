...
void Custom::handleEvent(const sf::Event &e) {
    ...
    if (this->level1_btn->handleInput(position, e)) {
        this->states->push(std::make_unique<GameState>(this->window, this->states, "data/custom/custom_level_0.json"));
    }
    if (this->level2_btn->handleInput(position, e)) {
        this->states->push(std::make_unique<GameState>(this->window, this->states, "data/custom/custom_level_1.json"));
    }
    if (this->level3_btn->handleInput(position, e)) {
        this->states->push(std::make_unique<GameState>(this->window, this->states, "data/custom/custom_level_2.json"));
    }
    if (this->level4_btn->handleInput(position, e)) {
        this->states->push(std::make_unique<GameState>(this->window, this->states, "data/custom/custom_level_3.json"));
    }
    if (this->level5_btn->handleInput(position, e)) {
        this->states->push(std::make_unique<GameState>(this->window, this->states, "data/custom/custom_level_4.json"));
    }
